...
class SeqStoreMongo:
    ...
    def catalog_proteins(self, proteins: list):
        from modelseedpy_ext.re.cdm.core import generate_cdm_uuid
        h_to_protein = {p.hash_value: p for p in proteins}
        cursor = self.collection.find({'_id': {'$in': list(h_to_protein)}})
        found = {doc['_id'] for doc in cursor}
        to_add = set(h_to_protein) - found
        if len(to_add) > 0:
            self.collection.insert_many([{"_id": h, "z_seq": h_to_protein[h].z_compress()} for h in to_add])
        cursor = self.collection_cdm.find({'_id': {'$in': list(h_to_protein)}})
        found = {doc['_id'] for doc in cursor}
        to_add = set(h_to_protein) - found
        if len(to_add) > 0:
            self.collection_cdm.insert_many([{"_id": h, "cdm_id": generate_cdm_uuid('CDM:prot')} for h in to_add])
