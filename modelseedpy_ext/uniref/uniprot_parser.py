    ...
    def parse_section(self, elem, parser, tag_end, capture):
        ...
        for action, elem in parser:
            ...
            if action == 'end' and tag == self.ns + tag_end:
                ...
                if self.verbose:
                    print(f'[no capture] [section:{tag_end}] [tag:{self.tag}]', action, tag, elem.attrib, elem.text, elem.sourceline)
    ...
    def parse_blob(self, elem, parser, tag_end):
        ...
        for action, elem in parser:
            ...
            if action == 'end' and tag == self.ns + tag_end:
                ...
                if self.verbose:
                    print(f'[no capture parse_blob] {self.tag}', action, tag, elem.attrib, elem.text, elem.sourceline)
    ...
    def parse_source(self, elem, parser):
        ...
        for action, elem in parser:
            ...
            if action == 'end' and tag == self.ns + 'source':
                ...
                print(f'[no capture source] {self.tag}', action, tag, elem.attrib, elem.text)
    ...
    def parse_name(self, elem, parser, end):
        ...
        for _action, _elem in parser:
            ...
            if _action == 'end' and _tag == self.ns + end:
                ...
                print(f'[no _capture name] {_tag}', _action, _tag, _elem.attrib, _elem.text)
    ...
    def parse_component(self, elem, parser):
        ...
        for action, elem in parser:
            ...
            if action == 'end' and tag == self.ns + 'component':
                ...
                res['recommended_name'].append(self.parse_name(elem, parser, 'recommendedName'))
            elif action == 'start' and tag == self.ns + 'alternativeName':
                res['alternative_name'].append(self.parse_name(elem, parser, 'alternativeName'))
            else:
                if self.verbose:
                    print(f'[no capture component] {self.tag}', action, tag, elem.attrib, elem.text)
...
class CaptureComment(Wut):
    ...
    def parse_isoform(self, elem, parser):
        ...
        for action, elem in parser:
            ...
            if action == 'end' and tag == self.ns + 'isoform':
                ...
                if self.verbose:
                    print(f'[no capture parse_isoform] {self.tag}', action, tag, elem.attrib, elem.text, elem.sourceline)
    ...
    def parse_conflict(self, elem, parser):
        ...
        for action, elem in parser:
            ...
            if action == 'end' and tag == self.ns + 'conflict':
                ...
                if self.verbose:
                    print(f'[no capture parse_conflict] {self.tag}', action, tag, elem.attrib, elem.text, elem.sourceline)
    ...
    def parse_location_section(self, elem, parser):
        capture = {
            'start': {
                self.ns + 'position': (self.parse_position, list, 'position')
            }
        }
    ...
    def parse(self, elem, parser):
        ...
        capture = {
            'start': {
                ...
                self.ns + 'temperatureDependence': (self.parse_temperature, list, 'temperatureDependence'),
                self.ns + 'cofactor': (self.parse_cofactor, list, 'cofactor'),
                self.ns + 'kinetics': (self.parse_kinetics, list, 'kinetics'),
                self.ns + 'subcellularLocation': (self.parse_subcellular_location, list, 'subcellularLocation'),
                self.ns + 'reaction': (self.parse_reaction, list, 'reaction'),
                self.ns + 'physiologicalReaction': (self.parse_physiological_reaction, list, 'physiologicalReaction'),
                ...
                self.ns + 'organismsDiffer': (self.parse_organisms_differ, list, 'organismsDiffer')
            }
        }
        for action, elem in parser:
            tag = elem.tag
