...
class GSPComBuilder:
    ...
    def external_denitri(self, model):
        ...
        delete = {
            'rxn10577_cR', 'rxn10577_cA', 'rxn08966_cA',
            'rxn08966_eR', 'rxn09008_cR', 'rxn09008_cA',
            'rxn05890_cR', 'rxn11932_cA', 'rxn11932_cR',
            'rxn05627_cR', 'rxn05627_cA', 'rxn05625_cA'
        }
        model.remove_reactions(delete)
        model.add_metabolites([
            Metabolite('cpd00418_e0', 'NO', 'NO_e0')
        ])
        ...
        ex.add_metabolites({
            model.metabolites.cpd00418_e0: -1
        })
        ...
        for model, token in self.models.items():
            ...
            for r in model.reactions:
                if not r.id.startswith('EX_'):
                    ...
                    r_copy_m = dict(map(lambda x: (model_com.metabolites.get_by_id(m_pointer[x[0].id]), x[1]), r.metabolites.items()))
    ...
    def media_fix(self, model_com, media_com):
        ...
        for ex_id, v in model_com.medium.items():
            ...
            print(list(filter(lambda x :x['id'] == compound_id + '_e0', model_o['modelcompounds']))[0]['compound_ref'])
            mediacompounds.append({'compound_ref': '489/6/15/compounds/id/' + compound_id,
                                   'concentration': 20,
                                   'id': compound_id,
                                   'inchikey': '',
                                   'maxFlux': v,
            ...
            if r.id not in model2.reactions:
                ...
                if len(r.metabolites) > 1:
                    ...
                    modelReactionReagents = list \
                        (map(lambda x: {'coefficient': x[1], 'modelcompound_ref': '~/modelcompounds/id/' + x[0].id}, r.metabolites.items()))
