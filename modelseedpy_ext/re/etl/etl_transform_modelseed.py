...
class ETLTransformModelSEEDCompound(ETLTransformGraph):
    ...
    def transform(self, compound_data):
        ...
        node_compound = add_node(node_compound_key, 'modelseed_compound', data=node_compound_data)

        if 'smiles' in compound_data and len(compound_data['smiles']) > 0:
            try:
                biochem_compound = self.biochem_service.get_molecule(compound_data['smiles'])
                ...
                node_cdk_smiles = add_node(get_hash(biochem_smiles), 're_biochem_smiles',
                                           data={'cdk_inchi_smiles': True, 'value': biochem_smiles})
                if compound_smiles != biochem_smiles:
                    node_smiles = add_node(get_hash(compound_smiles), 're_biochem_smiles',
                                           data={'value': compound_smiles})
                    add_edge(node_smiles, node_cdk_smiles, 're_biochem_smiles_equivalent_smiles')
                    add_edge(node_compound, node_smiles, 'modelseed_compound_has_smiles')
                else:
                    add_edge(node_compound, node_cdk_smiles, 'modelseed_compound_has_smiles')

                node_inchi = add_node(get_hash(biochem_inchi), 're_biochem_inchi', data={'value': biochem_inchi})
                node_inchikey = add_node(biochem_inchi_key, 're_biochem_inchikey', data={})
                ...
                add_edge(node_compound, node_inchikey, 'modelseed_compound_has_inchi_key')
                add_edge(node_compound, node_inchi, 'modelseed_compound_has_inchi')
            except ValueError as e:
                e
        if 'linked_compound' in compound_data and compound_data['linked_compound'] != None:
