...
class KEGenome:
    def __init__(
        ...
        if self.contig_data:
            for contig in self.contig_data.features:
                self.contigs[contig.id] = [
                    contig.seq,
                    str(Seq(contig.seq).complement()),
                ]
    ...
    def load_from_kbase(kbase, object_id, ws, kb_re, rast, load_arango, transform_rast):
        ...
        kbase.download_file_from_kbase2(
            assembly.fasta_handle_ref,
            "/home/fliu/kbase/cache/handle/" + assembly.fasta_handle_ref,
        )
        ms_contigs = MSGenome.from_fasta(
            "/home/fliu/kbase/cache/handle/" + assembly.fasta_handle_ref, split=" "
        )

        return KEGenome(
            genome, assembly, ms_contigs, rast, kb_re, load_arango, transform_rast
        )
    ...
    def etl_rast(self, h_list, batch_size=20):
        ...
        for i in h_list:
            ...
            if len(q) >= batch_size:
                p_features = [
                    {"id": x[0], "protein_translation": x[1]} for x in q.items()
                ]
                ...
                self.load_arango.load({"nodes": _nodes, "edges": _edges})
                q = {}
        if len(q) > 0:
    ...
    def get_loc_pos(self, skip_seq_match=False):
        ...
        for f in self.genome.features:
            ...
            if len(ll) == 0:
                ...
                if p_min >= p_max:
                    raise ValueError(
                        f"assertion fail for p_min < pmax: ({p_min}, {p_max})"
                    )
