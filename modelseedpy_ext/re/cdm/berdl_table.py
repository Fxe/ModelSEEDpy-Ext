...
data_features = []
for f in genome_query.features:
    contig, start, end, strand = get_coords_genemark(f)
    if contig not in assembly_query.features:
        print('@!')
    ...
    if f.seq:
        ...
        if annotation_bakta:
            ontology_bakta = []
            if annotation_bakta:
                ontology_bakta = collect_ontology(annotation_bakta)
            for kind, value in ontology_bakta:
                if kind not in annotation:
                    annotation[kind] = []
                annotation[kind].append(value)
        ...
        # if len(rast_annotation) > 0:
        #    annotation['rast'] = '\t'.join(rast_annotation)
        row = GenomeFeature(genome_id=kbase_genome_id,
                            contig_id=contig_id,
                            feature_id=f.id,
                            length=len(seq),
                            start=start,
                            ...
                            strand=strand,
                            sequence=seq,
                            sequence_hash=protein_hash,

                            pangenome_cluster_id=annotation.get('ke_pan_genome_cluster_id'),
                            pangenome_is_core=annotation.get('ke_pan_genome_is_core'),

                            bakta_function='; '.join(annotation.get('bakta_product', [])),
                            psortb=annotation.get('psortb'),
                            gene_names='; '.join(annotation.get('gene_names', [])),
                            go='; '.join(annotation.get('go', [])),
                            ko='; '.join(annotation.get('ko', [])),
                            pfam='; '.join(annotation.get('pfam', [])),
                            cog='; '.join(annotation.get('cog', [])),
                            ec='; '.join(annotation.get('ec', [])),
                            so='; '.join(annotation.get('so', [])),
                            rast_function=annotation.get(protein_hash),

                            uniref_100='; '.join(annotation.get('uniref_100', [])),
                            uniref_90='; '.join(annotation.get('uniref_90', [])),
                            uniref_50='; '.join(annotation.get('uniref_50', [])),
                            )
        data_features.append(row)
    # data_features.append(RowFeature(kbase_genome_id,
    #                                contig, f.id, len(f.seq),
