...
class NcbiEutils:
    ...
    def esearch(self, db, term, retmode='xml', retmax=10, retstart=0):
        ...
        res = requests.get(self.base_url + '/esearch.fcgi?' + self.build_url_params(params))
    ...
    def esummary(self, db, id, retmode='xml'):
        ...
        res = requests.get(self.base_url + '/esummary.fcgi?' + self.build_url_params(params))
        tree = ET.fromstring(res.content)
    ...
    def elink(self, dbfrom, db, ids):
        ...
        if self.api_key:
            params['api_key'] = self.api_key
        res = requests.get(self.base_url + '/elink.fcgi?' + self.build_url_params(params))
        return res